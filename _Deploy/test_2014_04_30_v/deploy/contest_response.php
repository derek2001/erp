	...
	if($i>0){
		...
		if ($_SESSION["user"]->db->select($sql0))
			...
			if ($_SESSION["user"]->db->select($sql1))
				...
				if($status=="C"){
					$sql20="UPDATE dbo.[order] set contest=0 WHERE id=$id_order";
					$_SESSION["user"]->db->select($sql20);
				}
				else{
					$sql20="UPDATE dbo.[order] set contest=1 WHERE id=$id_order";
					$_SESSION["user"]->db->select($sql20);
				}
...
if($_GET['type']==5){
	...
	if($i>0){
		...
		$psql="INSERT INTO dbo.[contest](id_order, notes)VALUES($id_order,'".$text."')";
       	if ($_SESSION["user"]->db->select($psql))
       		 $data = 0;
		else
			 $data = 1;
...
if($_GET['type']==7){
    ...
    $_SESSION["user"]->db->select($sql);
    $html='<table width="920" border="0" cellpadding="0" cellspacing="0">
                <tr>
                ...
                <tr>
                  ...
                  <td width="920" class="cell_tablecenter"><table width="920" border="0" cellpadding="0" cellspacing="1">
<tr><td width="100%" align="left">
<table width="100%" border="0">
<tr class="cell_reccolor_neutral_01" align="center">
                    <td width="100">&nbsp;</td>
                    <td width="300">Name</td>
					<td width="300">Created By</td>
                    <td width="20">Date</td>
                    <td width="10">&nbsp;</td>
                </tr>';
	$i=0;
   while ( $row = $_SESSION["user"]->db->fetchArray() ){
	...
	$html2='<table width="920" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td class="cell_table_lefttophard_01"><img src="gfx/spacer.gif" width="3" height="3"></td>
                  <td width="333" class="cell_table_top"><img src="gfx/spacer.gif" width="333" height="3"></td>
                  <td class="cell_table_righttophard_01"><img src="gfx/spacer.gif" width="3" height="3"></td>
