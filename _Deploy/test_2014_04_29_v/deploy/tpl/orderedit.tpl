...
<script type="text/javascript">
    $p = jQuery.noConflict();
    var status = "{$unit_status}";
    {literal}
    $p(document).ready(function() {
        $p('#unit_status input:radio').change(function() {
            var selectedVal = $p("#unit_status input:radio:checked").val();
            if(4 == selectedVal){
                $p('#unitList').html(status);
            }
        });
        /*$p('#unit_status input:radio').click(function() {
            var selectedVal = $p("#unit_status input:radio:unchecked").val();
                $p('select').remove();
                $p('#unitList').hide(status);
        });*/
    });
    {/literal}
</script>
...
function save_templater(){
    if (req.readyState == 4) {
        if (req.status == 200){
			...
			document.getElementById('field_com').value="";
...
function Validate(){
	if(document.getElementById('field_com').value ==''){
        alert('This field is Requiered');
        return false;
    }
...
function ChangeCheck(pValue){
	if(pValue==1){
		document.getElementById('td_n').style.display = "none";
		document.getElementById('td_y').style.display = "block";
	}
	else{
		document.getElementById('td_y').style.display = "none";
		document.getElementById('td_n').style.display = "block";
	}
...
function SelectType(){
	if(document.getElementById('types').value != -1){
		if(document.getElementById('types').value=='T')
			document.getElementById('templator').value=document.getElementById('field_com').value;
		
		if(document.getElementById('types').value=='I')
			document.getElementById('installer').value=document.getElementById('field_com').value;
...
<tr class="cell_reccolor_yellow_01a" style="display:{if $id==''}none;{/if}">
        <td>Contest:</td>
        <td>
         {if $is_contest_status == 'C'}
         	 <table width="40px" border="0" cellspacing="0" cellpadding="0">
              <tr class="cell_reccolor_yellow_01a">
                <td><input type="checkbox" name="contest_chck" id="contest_chck" disabled="disabled"></td>
                <td id="td_c" align="center">Cancel</td>
              </tr>
            </table>
         {else}
          <table width="40px" border="0" cellspacing="0" cellpadding="0">
          <tr class="cell_reccolor_yellow_01a">
            <td><input type="checkbox" name="contest_chck" id="contest_chck" {if $is_contest eq 1}checked disabled="disabled"{/if} onClick="document.getElementById('contest').value= document.getElementById('contest_chck').checked?1:0;ChangeCheck(document.getElementById('contest').value);"></td>
                <td id="td_y" align="center" style="display:{if $is_contest eq 0}none;{/if}">Yes</td>
                <td id="td_n" align="center" style="display:{if $is_contest eq 1}none;{/if}">No</td>
          </tr>
        </table>
         {/if}
          </td>
...
<br>
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   <input type="submit" name="submit_o" id="save_button" class="BUTTON_OK" value="Save"{if $editform == false or $wo_unit_lock==true} disabled{/if}>
...
<tr valign="top">
    ...
    {if $id<>''}
        {if $stats!=''}
        {assign var="table_width" value="500"}
...
<tr>
    ...
    {assign var="table_width" value="920"}
    {include file="table_header.tpl"} 
     <tr class="cell_reccolor_neutral_01">
            <td colspan="2" width="70" align="left">
            <table width="200" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><input type="text" id="field_com" size="40" maxlength="200"></td>
    <td><select id="types" onchange="SelectType();">
                  <option value="-1" selected="selected">Select Option</option>
                  <option value="T">Templator</option>
                  <option value="I">Installer</option>
                </select></td>
    <td>&nbsp;<input type="button" value="Save Log" class="BUTTON_OK" onclick="Save({$data.id});"><input type="hidden" id="templator" size="40" maxlength="200"> <input type="hidden" id="installer" size="40" maxlength="200"> </td>
  </tr>
</table>
